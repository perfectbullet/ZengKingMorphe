...
try:
    ...
    logger.error("Division by zero error", error=str(e), exc_info=True)

# Test 3: Warning with multiple context fields
print("\n=== Test 3: Warning with multiple fields ===")
logger.warning("High memory usage", memory_mb=512, threshold_mb=400, process_id=9999)
...
logger.debug("Request received", method="POST", url="/api/chat", payload_size=1024)

# Test 6: Simulating conversation service usage pattern
print("\n=== Test 6: Simulating conversation_service.py pattern ===")
logger.info("Using Ollama LLM", model="qwen2.5:7b")
logger.info("Employee config loaded", employee_id="emp_001", kb_count=3, faq_count=5)
logger.error("Failed to load config", error="Connection timeout", exc_info=False)
...
print("🎉 The logger.info('msg', key=value) syntax works perfectly!")
