...
async def list_recent_jobs(
    ...
    try:
        ...
        logger.info(f"Retrieved {len(jobs)} recent jobs", limit=limit)
        return jobs
...
async def get_job_detail(job_id: str):
    ...
    try:
        ...
        logger.error(f"Failed to get job detail: {job_id}, error: {str(e)}", exc_info=True)
...
async def get_job_markdown(job_id: str):
    ...
    try:
        ...
        # 从 final_result 中提取 Markdown 内容
        final_result = job.get("final_result", {})
        ...
        logger.error(f"Failed to get job markdown: {job_id}, error: {str(e)}", exc_info=True)
...
async def mineru_view_page():
    ...
    提供一个 Web 界面查看最近的解析任务和 Markdown 结果。
    """
    html_content = """
<!DOCTYPE html>
<html lang="zh-CN">
    ...
    <script>
        ...
        async function selectJob(jobId, fileName) {
            ...
            try {
                ...
                // 渲染 Markdown（简单渲染，实际项目可使用 marked.js）
        ...
        // 简单的 Markdown 渲染（生产环境建议使用 marked.js 或类似库）
