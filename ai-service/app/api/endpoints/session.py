...
# 初始化同步服务
employee_sync_service = EmployeeSyncService()
...
async def create_session(
    ...
    **响应数据**：
    ...
    try:
        ...
        # Step 1: 从 Java API 获取并同步员工数据
        synced_employee_id = await employee_sync_service.fetch_and_sync(request.employee_id, db)

        if synced_employee_id:
            logger.info(f"员工数据同步成功: employee_id={synced_employee_id}")
        else:
            logger.warning(f"员工数据同步失败: employee_id={request.employee_id}")

        # Step 2: 检查员工是否存在（从同步或已有数据）
