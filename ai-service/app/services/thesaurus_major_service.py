"""
ThesaurusMajor processing service.
"""
...
class ThesaurusMajorProcessor:

    async def thesaurus_major_vectorization(self, task_id: str, thesaurus_id: str):
        """
            处理流程：
            1. 从MongoDB的专业词库集合按employee_id查询词库的词条数据
        ...
        try:
            thesaurus = await db.thesaurus_major.find_one({"thesaurus_id": thesaurus_id})
            if not thesaurus:
                logger.warning(f"thesaurus_major_vectorization not found: task_id={task_id}")
                return
            ...
            logger.error(f"thesaurus_major_vectorization exception: task_id={task_id} error={str(e)}", exc_info=True)
            raise
    ...
    async def _thesaurus_vectorization(self, thesaurus: Any):
        ...
        try:
            ...
            if not update_time or update_time.strip() == "":
                ...
                logger.info(f"_thesaurus_vectorization thesaurus_id={thesaurus_id} already synced (update_time: {update_time}, "
            ...
            if synced_at and needs_update:
                logger.info(f"_thesaurus_vectorization thesaurus_id={thesaurus_id} needs update (update_time: {update_time}, "
            ...
            logger.error(f"_thesaurus_vectorization exception: thesaurus_id={thesaurus_id} error={str(e)}", exc_info=True)
            raise

    async def delete_thesaurus_vectorization_data(self, thesaurus_id: str):
        logger.info(f"delete_thesaurus_vectorization_data is disabled and delete chroma_db: thesaurus_id={thesaurus_id}", exc_info=True)
        ...
        try:
            ...
            logger.warning(f"delete_thesaurus_vectorization_data ElasticSearch delete failed: thesaurus_id={thesaurus_id} "
                           f"error={str(e)}", exc_info=True)
            raise

    async def create_thesaurus_vectorization_data(self, thesaurus: Any, update_time: str):
        thesaurus_id = thesaurus["thesaurus_id"]
        logger.info(f"create_thesaurus_vectorization_data is disabled and delete chroma_db: thesaurus_id={thesaurus_id}", exc_info=True)

        # 3、提取要进行向量化的文本数据单元
        combined_text = thesaurus.get("combined_text", "").strip()

        if not combined_text:
            logger.info(f"create_thesaurus_vectorization_data thesaurus_id={thesaurus_id} has empty combined_text, skipping", exc_info=True)
            return
        ...
        try:
            ...
            if thesaurus.get("vector_id"):
                try:
                    ...
                    logger.warning(f"create_thesaurus_vectorization_data Failed to delete existing FAQ vector: thesaurus_id= {thesaurus_id} "
                                   f"error={str(e)}", exc_info=True)
            ...
            logger.error(f"create_thesaurus_vectorization_data Failed to write FAQ to ChromaDB: thesaurus_id={thesaurus_id} "
                         f"error={str(e)}", exc_info=True)
            raise
        ...
        try:
            es_doc = {
                ...
                "thesaurus_name": thesaurus.get("thesaurus_name", ""),
                "word_name": thesaurus.get("word_name", ""),
                "similar_words": thesaurus.get("similar_words", []),
...
# Global thesaurus_major processor instance
thesaurus_major_processor = ThesaurusMajorProcessor()
