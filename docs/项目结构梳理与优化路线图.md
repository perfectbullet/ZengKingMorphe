...
/home/zj/miniconda3/envs/morphe/bin/python -m tests.test_chat_stream_v2 -q "解释一下导数的概念" --host http://localhost:8100
/home/zj/miniconda3/envs/morphe/bin/python -m tests.test_chat_stream_v2 -q "负三的值阿巴阿巴阿巴" --host http://localhost:8100
```

该入口调用已启动的 v2 HTTP 流式服务。数学题应检查路由、转换后的题干、答案正确性及公式格式；非数学题不应误路由到数学模型；噪声样例按闸门实际结果判断，不硬编码为必定拒答。调试产物写入忽略目录。
