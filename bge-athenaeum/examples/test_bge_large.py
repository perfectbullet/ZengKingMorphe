...
重要限制: BGE-Large 最大支持 512 字符（约 512 tokens）

运行测试: python test_bge_large.py
...
class BGELargeEmbeddingClient:
    """BGE-Large-ZH 嵌入模型客户端 (使用 OpenAI SDK)

    重要限制: 最大支持 512 字符（约 512 tokens）
    超过限制会报错: This model's maximum context length is 512 tokens
    """

    def __init__(
        self,
        base_url: str = "http://192.168.8.233:8093",
        api_key: str = "not-needed",
        timeout: int = 120,
    ):
        self.base_url = base_url.rstrip("/")
        self.timeout = timeout
        self.model_name = "BAAI/BGE-large"
        # 初始化 OpenAI 客户端
        self.client = OpenAI(
            api_key=api_key,
    ...
    def embed(
        ...
        重要限制: 每个文本最大 512 字符（约 512 tokens）
    ...
    def test_long_text_embedding(
        self,
        lengths: List[int] = [100, 200, 300, 400, 500, 512],
    ) -> None:
        """
        测试不同长度的中文文本嵌入
        ...
        print(f"\n{'=' * 60}")
        print("BGE-Large 长文本嵌入测试")
        print("重要限制: 最大支持 512 字符（约 512 tokens）")
        print("实测中文约 1 tokens/字符")
        print(f"{'=' * 60}")
    ...
    def test_chinese_semantic_search(self) -> None:
        ...
        knowledge_base = [
            ...
            "深度学习是机器学习的一个子领域，使用多层神经网络处理数据。",
            "自然语言处理（NLP）是AI的一个领域，专注于计算机与人类语言的交互。",
            "计算机视觉使机器能够从图像和视频中提取有意义的信息。",
            "Python是一种广泛用于机器学习和数据科学的高级编程语言。",
            "今天北京的天气很好，阳光明媚。",
            "我昨天去了一家很好的餐厅，吃了美味的火锅。",
        ]
        ...
        for query in queries:
            print(f"\n查询: {query}")
            results = self.similarity(query, knowledge_base, top_k=3)
            print(f"Top 3 相关文档:")
...
if __name__ == "__main__":
    print("\n" + "=" * 60)
    print("BGE-Large-ZH Embedding 服务测试 (OpenAI SDK)")
    print("=" * 60)

    # 初始化客户端
    client = BGELargeEmbeddingClient()  # 默认连接到 http://192.168.8.233:8093
    ...
    # 示例 6: 长文本嵌入测试
    print("\n[示例 6] 长文本嵌入测试")
    print("重要: BGE-Large 最大支持 512 字符")
    client.test_long_text_embedding([100, 200, 300, 400, 500, 512])

    # 示例 7: 中文语义搜索测试
    print("\n[示例 7] 中文语义搜索测试")
    client.test_chinese_semantic_search()

    # 示例 8: 性能对比测试
    print("\n[示例 8] 性能测试 (单次 vs 批量)")
    test_texts = ["测试文本{}".format(i) for i in range(10)]
    ...
    print(f"  单次调用 10 个文本: {single_time:.2f}秒")

    # 批量调用
    start_time = time.time()
    client.embed(test_texts)
    batch_time = time.time() - start_time
    print(f"  批量调用 10 个文本: {batch_time:.2f}秒")
    print(f"  批量调用加速比: {single_time / batch_time:.2f}x")
