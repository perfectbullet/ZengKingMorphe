...
=====================================
手动教材数学概念 -> LightRAG「自定义 KG」导入脚本（推荐路径）。
...
- 本脚本使用 rag.ainsert_custom_kg(...)，把每条人工概念记录直接作为自定义 KG
  写入，完全不走 LLM 实体抽取，图中只保留 MANUAL_MATH_CONCEPT 实体。

每条 JSONL 记录转成：
- 1 个 chunk（content=md_content, source_id=doc_id）
- 1 个 entity（entity_name=concept_name, entity_type=MANUAL_MATH_CONCEPT,
...
本脚本是独立验证脚本，不依赖也不修改 ai-service 业务代码或 LightRAG 源码。
"""
...
def build_rag(working_dir: Path, llm: dict, emb: dict) -> LightRAG:
    """custom KG 导入不触发 LLM 实体抽取，entity_types 仅供查询侧语义一致。"""
...
def build_record_custom_kg(
    ...
    """单条记录 -> 一个 chunk + 一个 MANUAL_MATH_CONCEPT 实体（无 relationships）。
...
def _prepare_records(
    ...
    for idx, raw_item in enumerate(items, 1):
        ...
        file_path = default_file_path(raw_item) or f"manual_math_concepts/{concept_name}.md"
        source_type = raw_item.get("source_type") or "manual_math_concept"
        ...
        logger.info(
            ...
            f"file_path={file_path} | source_type={source_type}"
        )
...
async def run(args: argparse.Namespace) -> int:
    ...
    # 构造每条记录的 custom_kg
    prepared = _prepare_records(items, whitelist, args, stats)
    logger.info(
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="导入手动数学概念到 LightRAG（custom KG 模式，不走 LLM 实体抽取）"
    )
    p.add_argument(
        "--config",
        required=True,
