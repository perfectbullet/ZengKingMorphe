...
- score 仅表示程序召回候选时的排序分数，不代表该候选一定是正确锚点。
- 必须比较全部候选，不能机械选择 score 最高、行号最小或距离章标题最近的候选。

章内小目录定义：
教材正文可能在章标题后连续列出本章各节名称，例如：
...
这类连续出现、彼此之间没有实质正文的同级标题列表属于“章内小目录”，不是各节正文真正开始的位置，不得作为目录锚点。

章内小目录通常具有以下一个或多个特征：
1. 位于章标题之后较近的位置。
2. 连续出现两个或多个“第一节、第二节、第三节”等同级标题。
3. 相邻节标题之间没有正文、步骤、图表说明或知识内容展开。
4. 相同或高度相似的标题在后文再次出现。
5. 后文再次出现的标题之后才有该节对应的连续正文、步骤、图片说明、案例或下级标题。
6. 候选的 context_before 或 context_after 中出现多个连续同级节标题。

真实正文锚点通常具有以下特征：
1. 优先是 Markdown 标题，例如“## 第一节 硬蜡技法”。
2. 正文可能省略章节编号，例如目录是“第一节 硬蜡技法”，正文标题可能是“## 硬蜡技法”。
3. 标题之后开始该节的正文、制作步骤、案例、图表说明或下级内容。
4. 不属于章标题后的连续节标题预览列表。
5. 位于该节内容实际开始的位置，而不是章节开头的目录预览位置。

严格规则：
1. 不允许自由生成行号，只能原样选择一个 candidate_id 及其 line_no。
2. 目录和正文可能有 OCR 差异，应优先依据章节编号，其次依据标题语义、candidate_type 和上下文。
3. 当章内小目录候选与后文正式正文标题候选同时存在时，必须选择后文正式正文标题。
4. “距离章标题更近”不代表更可能是正文锚点；章标题后的连续节标题通常是章内小目录。
5. Markdown 标题通常优先于普通短文本，但如果 Markdown 标题本身明显属于连续章内小目录，也必须排除。
6. 正文标题省略“第一节、第二节”等编号时，只要标题语义一致、上下文表明确实开始正文，可以匹配该 Markdown 标题。
7. 必须检查候选的 context_before 和 context_after。若候选前后主要是连续同级节标题，且没有实质正文展开，应判定为章内小目录候选。
8. 不能因为某个候选 score 更高，就忽略后文更符合真实正文起点特征的候选。
9. 若全部可靠候选都只是章内小目录、目录预览或普通文本，未找到真实正文起始标题，必须返回 matched=false，reason 明确写明“仅发现章内小目录，未找到可靠的正文标题锚点”。
10. 页码、图号、STEP 编号、图片文件名、视频名称、表格名称、案例中的同名短语不能作为目录标题锚点。
11. 没有可靠候选时 matched=false，相关字段使用 null 或空字符串。
12. catalog_index 必须原样返回。
13. 必须输出下面列出的全部 7 个字段，不能改名、不能省略。
14. 只输出严格 JSON，不要输出 Markdown 或额外解释。

选择优先级：
1. 不属于章内小目录、编号和标题均匹配的 Markdown 标题。
2. 不属于章内小目录、标题语义匹配但省略章节编号的 Markdown 标题。
3. 不属于章内小目录、编号和标题均可靠匹配的独立普通文本标题。
4. 没有可靠正文标题时返回 matched=false。
...
输出格式示例：
{
  ...
  "confidence": 0.94,
  "reason": "编号和标题匹配；候选为正式 Markdown 标题，附近存在正文展开，不属于章内小目录"
}
"""
